...
@nnx.jit(static_argnames=("hidden_dim", "head_dim", "num_heads"))
def create_sharded_attention(hidden_dim, head_dim, num_heads):
...
def megatron_attention_parallelism():
    ...
    with jax.profiler.trace('/tmp/jax-trace', create_perfetto_link=True, create_perfetto_trace=True):
        with mesh:
            attention = create_sharded_attention(hidden_dim, head_dim, num_heads)
